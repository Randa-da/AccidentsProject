...
SELECT "Country",count(*) AS "accident_count"
FROM road_accident_dataset
GROUP BY  "Country"
ORDER BY"accident_count" DESC
	
-- Looking at what time of day has the highest number of accidents
	
SELECT  "Time of Day" , count(*) as "accidents_time_counts" 
FROM road_accident_dataset
GROUP BY  "Time of Day"
ORDER BY  "accidents_time_counts" DESC
	  
-- Looking at accidents causes Counts
	
SELECT   "Accident Cause" ,count(*) AS "accident_cause_count"
FROM  road_accident_dataset
GROUP BY  "Accident Cause"

--Looking at accident severity count
	
SELECT "Accident Severity" ,count(*) AS "accident_severity_count"
FROM  road_accident_dataset
...
SELECT ROUND(count(*)FILTER(WHERE "Driver Alcohol Level" > 0.08)*100.0/count(*)) 
FROM road_accident_dataset
...
SELECT  "Driver Age Group",count(*) AS "accident_count"
FROM  road_accident_dataset
...
SELECT  AVG("Emergency Response Time")
FROM road_accident_dataset
